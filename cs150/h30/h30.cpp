...
// Add your code here
// Image() = default
Image::Image(const string&path){ load(path);}
//Image(const std::string& path){}
Image::Image(unsigned width, unsigned height): m_width(width), m_height(height),m_pixels(width*height){}//math
//explicity initializing var

//accessors constructed
unsigned Image::width() const{return m_width;}
unsigned Image::height() const{return m_height;}
unsigned Image::size() const{return m_pixels.size();}

Pixel* Image::begin()
{
    if(size()> 0 ) {return &m_pixels.front();}// check to see if pixels
    return nullptr;
}
Pixel* Image::end(){
    if(size() > 0 ){return begin() + size();}
    return nullptr;
}
//setters
void Image::fill(const Pixel& color){for( auto& p : m_pixels) p = color;}
...
bool Image::load(const std::string& path)
{
    ...
    UC* temp = stbi_load(path.c_str(),&w,&h,&bpp,BPP);//laoding a balue into temp
    if(temp == nullptr){return false;}
    Pixel* data = reinterpret_cast<Pixel*>(temp);
    m_width =w;
    m_height = h;
    m_pixels.resize(w*h);// reallocates pixels
    //set hence load 
...
{
    auto pos = path.rfind(".");
    if(pos == string::npos){return false;}//we have nothing save
    string ext = path.substr(pos);
    for(auto& e : ext){e = tolower(e);}
    if(ext==".jpg"){return stbi_write_jpg(path.c_str(),width(),height(),4, reinterpret_cast<UC*>(&m_pixels.front()),100);}
    else if(ext == ".bmp"){return stbi_write_bmp(path.c_str(),width(),height(),4, reinterpret_cast<UC*>(&m_pixels.front()));}
    else if(ext==".png"){return stbi_write_png(path.c_str(),width(),height(),4, reinterpret_cast<UC*>(&m_pixels.front()), width()*4);}

return false;// exit out false bc file type undertiemreasfdgsfhjgfgj
}
